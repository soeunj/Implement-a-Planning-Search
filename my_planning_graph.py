...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        for effect_node in node_a1.action.effect_add:
            if effect_node in node_a2.action.effect_rem:
                return True
        for effect_node in node_a2.action.effect_add:
            if effect_node in node_a1.action.effect_rem:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        for effect_node in node_a1.action.effect_add:
            if effect_node in node_a2.action.precond_neg:
                return True
        for effect_node in node_a2.action.effect_add:
            if effect_node in node_a1.action.precond_neg:
                return True

        for effect_node in node_a1.action.effect_rem:
            if effect_node in node_a2.action.precond_pos:
                return True
        for effect_node in node_a2.action.effect_rem:
            if effect_node in node_a1.action.precond_pos:
                return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goals:
            ...
            s_list = enumerate(s_levels)
            for level, s_nodes in s_list:
                if node in s_nodes:
                    level_sum += level
                    break
        return level_sum
